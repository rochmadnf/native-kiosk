...
function poliCardTemplate(item) {
  ...
  return `
    ...
    >
      <div class="flex items-center justify-between border-b border-b-brand-border pb-4">
        <i class="ph ph-${extraItem.icon} text-2xl pointer-events-none"></i>
        <span title="Ruang ${item.kode_ruang}" class="size-5 rounded-full font-semibold border border-brand-old inline-flex p-2 items-center justify-center font-mono text-sm">
          ${item.kode_ruang}
        </span>
      </div>
  ...
  return `
    ...
    >
      ...
      <div class="grid grid-cols-3 gap-4 border-t border-brand-border py-4">
        ${poliList.data.map(poliCardTemplate).join("")}
      </div>
    </section>`;
}
