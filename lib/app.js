...
async function selectPoli(index) {
  ...
  console.log("State setelah pilih poli:", state);
  ...
  Swal.fire({
    title: responseAtrean.status ? responseAtrean.data.nomor_antrian : "Gagal",
    text: responseAtrean.status
      ? responseAtrean.message
      : "Server gagal menerima permintaan.",
    icon: responseAtrean.status ? "success" : "error",
    showConfirmButton: false,
    timer: 5000,
    timerProgressBar: true,
  });
  ...
  if (responseAtrean.status) {
    // Cetak Struk
    const printStruck = await printQueueNumber(responseAtrean.data);

    if (printStruck.status == 200) {
...
patientForm.addEventListener("submit", async (event) => {
  ...
  console.log("State setelah cek NIK:", state);
