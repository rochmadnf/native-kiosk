...
import { CHAT_ID, TOPIC_ID } from "../config/telegram.js";
...
// ── Wake Lock: cegah layar/komputer sleep selama app dibuka ────────────────
let _wakeLock = null;
...
async function requestWakeLock() {
  ...
  try {
    ...
    _wakeLock.addEventListener("release", () => {
      console.info("Wake lock dilepas.");
    });
  } catch (err) {
    console.error("Wake lock gagal diminta:", err);
  }
}

requestWakeLock();

// Wake lock otomatis lepas saat tab disembunyikan, minta lagi saat kembali terlihat
...
// ── Auto-refresh setelah 3 menit tanpa aktivitas ────────────────────────────
      ...
      <div class="space-y-2 border-t border-brand-border py-2">
        <div class="flex items-center gap-x-4 w-full justify-between">
          <div class="w-full">
            ...
            <p class="w-full text-base font-mono text-green-900 tracking-wider">${patient.nama}</p>
          </div>
          <div class="w-full">
            <label class="text-xs text-green-900 font-semibold">Usia</label>
            <p class="w-full text-base font-mono text-green-900">${hitungUsia(patient.tgl_lahir)}</p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-x-4 w-full justify-between">
          <div class="w-full">
            ...
            <p
              class="w-full text-base font-mono text-green-900 tracking-wider cursor-pointer hover:text-green-900/75"
              onclick="copyToClipboard('${patient.nik}')"
            >${patient.nik}</p>
          </div>
          <div class="w-full">
            <label class="text-xs text-green-900 font-semibold">No. HP</label>
            <p class="w-full text-base font-mono text-green-900 tracking-wider">${patient.no_telpon ? patient.no_telpon : "-"}</p>
          </div>
        </div>
        ...
        <div class="flex items-center gap-x-4 w-full justify-between">
          <div class="w-full">
            ...
            >${patient.no_kartu_jaminan ? patient.no_kartu_jaminan : "-"}</p>
          </div>
          <div class="w-full">
            <label class="text-xs text-green-900 font-semibold">Status Peserta</label>
            <p class="w-full text-base font-mono text-green-900 tracking-wider">${patient.kdProviderPst ? patient.ketAktif + " &mdash;  FASKES: " + patient.kdProviderPst.nmProvider : "Umum"}</p>
          </div>
        </div>
...
// ── Lock agar tombol poli tidak bisa diklik dua kali sekaligus ────────────
let _isSelectingPoli = false;

// ── Bantuan UI: kunci/lepas seluruh tombol poli ───────────────────────────
function setPoliButtonsLoading(activeIndex) {
  const buttons = document.querySelectorAll("#listPoli [data-poli-btn]");

  buttons.forEach((btn) => {
    btn.disabled = true;
    btn.classList.add("pointer-events-none");

    if (Number(btn.dataset.poliBtn) === activeIndex) {
      btn.classList.add("opacity-80");
      btn.dataset.originalContent = btn.innerHTML;
      btn.innerHTML = `
        <div class="flex flex-col items-center justify-center gap-2 py-6 w-full">
          <i class="ph ph-circle-notch text-2xl animate-spin pointer-events-none"></i>
          <span class="text-xs font-medium">Memproses...</span>
        </div>`;
    } else {
...
function resetPoliButtonsLoading() {
  const buttons = document.querySelectorAll("#listPoli [data-poli-btn]");

  buttons.forEach((btn) => {
    btn.disabled = false;
    btn.classList.remove("pointer-events-none", "opacity-40", "opacity-80");
...
async function selectPoli(index) {
  // Cegah klik ganda selama proses pendaftaran masih berjalan
  if (_isSelectingPoli) return;
  _isSelectingPoli = true;
  setPoliButtonsLoading(index);

  try {
    await selectPoliInner(index);
  } catch (err) {
    console.error("Error selectPoli:", err);
    await swalError(
      "Gagal",
    ...
    _isSelectingPoli = false;
    resetPoliButtonsLoading();
  }
}

async function selectPoliInner(index) {
  ...
  if (responseAtrean.status) {
    ...
    if (printStruck.status) {
      ...
      await swalSuccess("Berhasil", fwPoli.message, {
        showConfirmButton: true,
      }).then(async () => {
        const currentDate = new Date().toLocaleString();

        const message = `📝 <b>Pendaftaran #${currentDate}</b>
          😊 <b>Nama</b> : ${state.patient.nama}
          🪪 <b>NIK</b> : <code>${state.patient.nik}</code> -- <code>${state.kepesertaan.noKartu}</code>
          🪧 <b>Kepesertaan</b> : ${state.kepesertaan.kdProviderPst ? state.kepesertaan.ketAktif + " &mdash; " + state.kepesertaan.kdProviderPst.nmProvider : "Umum"}
          🏝️ <b>Beda Faskes?</b> : ${isFaskesPalu.nmppk === "KAMONJI" ? "Tidak" : "Ya"}
          🩺 <b>Poli</b> : <b>${state.poli.nama_dokter}</b> (${state.poli.nama_pemeriksaan})
          `;

        const resTele = await sendMessageTelegram(CHAT_ID, TOPIC_ID, message);

        if (resTele.ok) {
          console.info("telegram terkirim.");
        }
        window.location.href = settings.BASE_URL;
      });
    }
...
function poliCardTemplate(item, index) {
  ...
  return `
    <button
      ...
      class="flex flex-col text-green-900 cursor-pointer text-left gap-2 px-4 py-2 rounded-md border border-brand-border transition duration-200 hover:bg-brand-bg/75 disabled:cursor-not-allowed"
    >
