...
export const CheckNik = async (nik) => {
  ...
  const epn = API_BASE_ENDPOINT + ((nik.length === 16) ? "/pasien/nik/" : "/nabelo/cek-nik/") + nik;
...
export const getVisitorList = async (status = '',limit = 30) => {
  try {
    const response = await fetch(`${API_BASE_ENDPOINT}/antrean?search=&page=1&limit=${limit}&status=${status}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        ...theHeaders,
...
export const forwardToPoli = async (id_kunjungan, poli) => {
  try {
    const response = await fetch(`${API_BASE_ENDPOINT}/antrean/confirm/${id_kunjungan}`, {
      method: "PATCH",
